...
const CardUser = (props: Props) => {
    ...
    return (
        <div className={"bg-white p-5 rounded-2xl flex flex-row items-start gap-4 hover:scale-[101%] transition"}>
            <Avatar className={"w-[100px] h-[100px] cursor-pointer"} link={`${ROUTES.profile}/${user.id}`}>
                {user.avatar ? (
            ...
            <div className={" flex flex-col mt-2 ml-2"}>
                ...
                <div className={"flex flex-row gap-2 mt-2"}>
                    ...
                    {user.rolesForProject.length === 0 && <Typography affects={"muted"}>Роли отсутствуют</Typography>}
                </div>
                <div className={"flex flex-row gap-2 mt-2 "}>
                    <ComplimentMini
                        icon={"🤝"}
