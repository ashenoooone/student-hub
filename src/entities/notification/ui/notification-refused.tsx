...
export const NotificationRefused: FC<NotificationRefusedProps> = ({notification}) => {
  ...
  return (
    <div className={'flex flex-row gap-2 items-center space-y-2'}>
      ...
      <Typography className={'ml-auto'}>{notification.sender.login !== profile?.login ? `Вы отказали ${notification.sender.login}!` : 'Вас не добавили в проект!'}</Typography>
    </div>
  );
};