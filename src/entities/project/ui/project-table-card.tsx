  ...
  return (
    <Box key={project.id} className={"flex flex-row items-stretch gap-4"}>
      <div className={"w-1/2 flex flex-col gap-3"}>
        <Avatar
          className={
            "w-full !rounded-[12px] min-w-[205px] min-h-[245px] cursor-pointer"
          }
        ...
        >
          {project.avatar ? (
            ...
            <AvatarFallback className={"!rounded-[12px]"}>
              {project.name}
            </AvatarFallback>
          )}
        </Avatar>
          ...
          <div className={"flex flex-col gap-3"}>
            ...
            <div className={"flex flex-row gap-1"}>
              {project.actualRoles.slice(0, 3).map((role, index) => (
                <Badge
                  className={
                    "w-max max-w-[83px] text-ellipsis select-none hover:bg-blue-400"
                  }
                  key={index}
                >
                  {role.name}
              ...
              {project.actualRoles.length > 3 && (
                <Badge className={"w-max select-none hover:bg-blue-400"}>
                  ...
                </Badge>
              )}
            </div>
      ...
      <div className={"w-1/2 flex flex-col gap-2"}>
        ...
        <Typography affects={"link"}>
          {PROJECT_STATUS[project.status as Status]}
        </Typography>
        <Typography>{project.description}</Typography>
        <Typography affects={"large"} className={"text-sm mt-auto"}>
          Количество участников: {project.members.length}
        </Typography>
        <MembersList
          members={convertUserToMember(project.members)}
