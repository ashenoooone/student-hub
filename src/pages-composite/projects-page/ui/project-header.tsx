      ...
      >
        <div className={"w-2/3 flex flex-col gap-3"}>
          ...
          <div>
            ...
            {project.media && project.media?.urlSite && (
              <Link
                className={"flex items-center my-1 gap-1"}
                href={project.media.urlSite}
              >
                <GlobeIcon />
                <Typography affects={"link"}>
                  {project.media.urlSite}
                </Typography>
              </Link>
            )}
            {project.media && project.media?.githubLink && (
              <Link
                className={"flex items-center my-1 gap-1"}
                href={project.media.githubLink}
              >
                <GitHubLogoIcon />
                <Typography affects={"link"}>
                  {project.media.githubLink}
                </Typography>
              </Link>
            )}
            {project.media && project.media.stack && (
              <>
                <Typography className={"text-[16px] my-1"} variant={"h3"}>
                  Стек:
                </Typography>
